...
object Bbs extends Serializable{
  ...
  //Hbase信息
  val TABLE_NAME = "H_REG_CONTENT_BBS";
  val CF = "CONTENT_BBS";
  ...
  val zkHost = "dn1:2181,dn2:2181,dn3:2181,dn4:2181,dn5:2181,nn1:2181,nn2:2181" //node1,node2,data1,data2,data3
...
//    hconf.set("hbase.zookeeper.master", "dn3:60000")
    ...
    val sconf = new SparkConf().setAppName("indexBBS")
  ...
  def commitSolr(hbaseRDD: RDD[(ImmutableBytesWritable, Result)]) {
    ...
    println("开始索引时间：>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" + new Date() + "索引总条目：<<<<<<<<<<<<<<<<<<<<<<<<" + hbaseRDD.count())
