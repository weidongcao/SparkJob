...
object Kmeans extends Serializable {
  ...
  def main(Args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("RobinProject").setMaster("local[4]")
    ...
    println("Data have been read from the database count is:" + hBaseRDD.count())
    ...
    println("Performing the kmeans clustering for k =" + math.pow(10, 3).toInt)
    val kmeans = new KMeans()
      .setK(math.pow(10, 3).toInt)
      .setFeaturesCol("pca_features")
      .setPredictionCol("k_" + math.pow(10, 3).toInt)
    val model = kmeans.fit(pcaDF)
    val transformedDF = model.transform(pcaDF)
    clusterResults = clusterResults.join(transformedDF.select("ending_mac", "k_" + math.pow(10, 3).toInt), "ending_mac")
    //}

    /*val properties = new Properties()
    properties.put("user", "root")
    properties.put("password", "5431333")

    clusterResults.write.jdbc("jdbc:mysql://localhost:3306/dbs_robin_project", "cluster_result", properties)

*/
    val myConf = HBaseConfiguration.create()
    myConf.set("hbase.zookeeper.quorum", "node1,node2,data1,data2,data3") //修改成自己的Hbase的配置
