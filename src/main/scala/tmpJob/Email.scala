...
object Email {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val jarPath = "/usr/local/SparkJob.jar";

    //out\\artifacts\\SparkAction_jar\\SparkJob.jar
    ...
    //初始化SparkConf
    val sconf = new SparkConf().setAppName("indexEmail_tmp")
    //val sconf = new SparkConf().setAppName("indexEmail")
  ...
  def transFormDocs(result: Result, sDoc: SolrInputDocument) {
    ...
    sDoc.addField("MAILID", etl_field(Bytes.toString(result.getValue(CF.getBytes, "MAILID".getBytes))));
    sDoc.addField("SEND_TIME", etl_field(Bytes.toString(result.getValue(CF.getBytes, "SEND_TIME".getBytes))));
    sDoc.addField("MAIL_FROM", etl_field(Bytes.toString(result.getValue(CF.getBytes, "MAIL_FROM".getBytes))));
    sDoc.addField("MAIL_TO", etl_field(Bytes.toString(result.getValue(CF.getBytes, "MAIL_TO".getBytes))));
    sDoc.addField("CC", etl_field(Bytes.toString(result.getValue(CF.getBytes, "CC".getBytes))));
    sDoc.addField("BCC", etl_field(Bytes.toString(result.getValue(CF.getBytes, "BCC".getBytes))));
    sDoc.addField("SUBJECT", etl_field(Bytes.toString(result.getValue(CF.getBytes, "SUBJECT".getBytes))));
    sDoc.addField("SUMMARY", etl_field(Bytes.toString(result.getValue(CF.getBytes, "SUMMARY".getBytes))));
    sDoc.addField("ATTACHMENT", etl_field(Bytes.toString(result.getValue(CF.getBytes, "ATTACHMENT".getBytes))));
    sDoc.addField("FILE_PATH", etl_field(Bytes.toString(result.getValue(CF.getBytes, "FILE_PATH".getBytes))));
