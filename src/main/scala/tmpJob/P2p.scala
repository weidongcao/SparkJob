...
object P2p {
  ...
  //Hbase信息
  val TABLE_NAME = "H_REG_CONTENT_P2P_TMP";
  val CF = "CONTENT_P2P";
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sconf = new SparkConf().setAppName("indexP2P_tmp")
  ...
  def transFormDocs(result: Result, sDoc: SolrInputDocument) {
    ...
    sDoc.addField("CLIENT_ID", etl_field(Bytes.toString(result.getValue(CF.getBytes, "CLIENT_ID".getBytes))));
    ...
    sDoc.addField("DEST_MAC", etl_field(Bytes.toString(result.getValue(CF.getBytes, "DEST_MAC".getBytes))));
