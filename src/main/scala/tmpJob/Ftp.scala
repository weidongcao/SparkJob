...
object Ftp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //远程提交时，需要提交打包后的jar
    //val jarPath = "out\\artifacts\\SparkAction_jar\\SparkJob.jar"
    ...
    val sconf = new SparkConf().setAppName("indexFTP_tmp")
  ...
  def transFormDocs(result: Result, sDoc: SolrInputDocument) {
    ...
    sDoc.addField("FILE_NAME", etl_field(Bytes.toString(result.getValue(CF.getBytes, "FILE_NAME".getBytes))));
    ...
    sDoc.addField("IS_COMPLETED", etl_field(Bytes.toString(result.getValue(CF.getBytes, "SUMMARY".getBytes))));
