...
object Http {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sconf = new SparkConf().setAppName("indexHttp_tmp")
  ...
  def transFormDocs(result: Result, sDoc: SolrInputDocument) {
    ...
    sDoc.addField("COOKIE_PATH", etl_field(Bytes.toString(result.getValue(CF.getBytes, "COOKIE_PATH".getBytes))));
    sDoc.addField("UPLOAD_FILE", etl_field(Bytes.toString(result.getValue(CF.getBytes, "UPLOAD_FILE".getBytes))));
    sDoc.addField("DOWNLOAD_FILE", etl_field(Bytes.toString(result.getValue(CF.getBytes, "DOWNLOAD_FILE".getBytes))));
