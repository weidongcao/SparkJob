...
object Remote {
  ...
  //Hbase信息
  val TABLE_NAME = "H_REG_CONTENT_REMOTE_TMP";
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sconf = new SparkConf().setAppName("indexRemote_tmp")
  ...
  def transFormDocs(result: Result, sDoc: SolrInputDocument) {
    ...
    sDoc.addField("COMMAND", etl_field(Bytes.toString(result.getValue(CF.getBytes, "COMMAND".getBytes))));
    sDoc.addField("CMD_ANSWER", etl_field(Bytes.toString(result.getValue(CF.getBytes, "CMD_ANSWER".getBytes))));
    sDoc.addField("CMD_RESULT", etl_field(Bytes.toString(result.getValue(CF.getBytes, "CMD_RESULT".getBytes))));
