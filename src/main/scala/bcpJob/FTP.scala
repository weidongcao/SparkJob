...
object FTP {
  ...
  //Hbase信息
  val TABLE_NAME = "H_REG_CONTENT_FTP_TMP";
  val CF = "CONTENT_FTP";

  val INPUT = "/opt/bcp/ftp/*ftp_content.bcp";
  val DEL_INPUT = "/opt/bcp/ftp";
  ...
  def main(args: Array[String]) {
    ...
    def convert(fields: String) = {
      ...
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("PASSWORD"), Bytes.toBytes(field(7)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("FILENAME"), Bytes.toBytes(field(8)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("FILE"), Bytes.toBytes(field(9)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("OPTYPE"), Bytes.toBytes(field(10)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("REMOTEIP"), Bytes.toBytes(field(12)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("IS_COMPLETE"), Bytes.toBytes(field(11)))
      ...
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("DATA_SOURCE"), Bytes.toBytes(field(21)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("FILESIZE"), Bytes.toBytes(field(22)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("FILEURL"), Bytes.toBytes(field(field.length - 1).replace("|$|", "")))
    ...
    val localData = readFile.filter(_.split("""\|#\|""").length == 24).map(convert)
