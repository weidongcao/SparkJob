...
object HTTP {
  ...
  //Hbase信息
  val TABLE_NAME = "H_REG_CONTENT_HTTP_TMP";
  val CF = "CONTENT_HTTP";

  val INPUT = "/opt/bcp/http/*http_content.bcp";
  val DEL_INPUT = "/opt/bcp/http";
  ...
  def main(args: Array[String]) {
    ...
    def convert(fields: String) = {
      ...
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("ACTIONTYPE"), Bytes.toBytes(field(10)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("SUBJECT"), Bytes.toBytes(field(11)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("SUMMARY"), Bytes.toBytes(field(12)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("COOKIE"), Bytes.toBytes(field(13)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("ZIPPATH"), Bytes.toBytes(field(14)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("UPFILE"), Bytes.toBytes(field(15)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("DOWNFILE"), Bytes.toBytes(field(16)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("REMOTEIP"), Bytes.toBytes(field(17)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("REMOTEPORT"), Bytes.toBytes(field(18)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("COMPUTERIP"), Bytes.toBytes(field(19)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("COMPUTERPORT"), Bytes.toBytes(field(20)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("COMPUTERMAC"), Bytes.toBytes(field(21)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("CAPTIME"), Bytes.toBytes(field(22)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("ROOM_ID"), Bytes.toBytes(field(23)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("CHECKIN_ID"), Bytes.toBytes(field(24)))
      p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("MACHINE_ID"), Bytes.toBytes(field(25)))
    ...
    println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>CaoWeidong<<<<<<<<<<<<<<<<<<<<<<<<")
    val localData = readFile.filter(_.split("""\|#\|""").length == 27).map(convert)
