...
/**
  ...
  */
object BBS extends Serializable{

  //Hbase信息
  val TABLE_NAME = "H_REG_CONTENT_BBS_TMP";
  //列族
  val CF = "CONTENT_BBS";
  //  val INPUT = "/solrCloud/bcp/*bbs_content.bcp";
  val INPUT = "/opt/bcp/bbs/*bbs_content.bcp";
  ...
  def convert(fields: String) = {
    ...
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("URL"), Bytes.toBytes(field(8)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("HOST"), Bytes.toBytes(field(9)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("REFURL"), Bytes.toBytes(field(10)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("REFHOST"), Bytes.toBytes(field(11)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("POSTING_ID"), Bytes.toBytes(field(12)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("TITLE"), Bytes.toBytes(field(13)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("AUTHOR"), Bytes.toBytes(field(14)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("SOURCE"), Bytes.toBytes(field(15)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("ACTIONTYPE"), Bytes.toBytes(field(16)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("SUMMARY"), Bytes.toBytes(field(17)))
    p.addColumn(Bytes.toBytes(CF), Bytes.toBytes("FILE"), Bytes.toBytes(field(18)))
